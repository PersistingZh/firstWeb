	...
	export default {
		...
		methods: {
			...
			updateNow() {
				...
				//     key: 'update_path',
				// });
				this.updatePath = uni.getStorageSync('update_path');
				console.log(this.updatePath, 'updatePath')
				plus.io.resolveLocalFileSystemURL(
					...
					(e) => {
						...
						// let dtask = plus.downloader.createDownload(_that.url, {
						// 	filename: '_downloads/'
						// });
						...
						const downloadTask = uni.downloadFile({
							...
							success: (downloadResult) => {
								if (downloadResult.statusCode === 200) {
									this.updatePath = downloadResult.tempFilePath
									uni.saveFile({
										tempFilePath: downloadResult.tempFilePath,
										success: function(res) {
											console.log(res)
											uni.setStorageSync('update_path', res.savedFilePath);
										}
									});
								}
							},
			...
			install() {
				...
				// 	}
				// );
				plus.runtime.install(this.updatePath, {
					force: false
				}, function() {
