 ...
 * @param {*} 
 */
export function isExistChar(value, target) {
	let values = (value || '').trim();
	const reg = /[\u4e00-\u9fa5]/ig;
	if (value) {
		if ((reg.test(value))) {
			target = value.replace(reg, '')
		}
	}
 ...
 * @param {*} fn 执行的函数
 * @param {*} param 执行函数的参数
 * @param {*} time 执行延迟的时间 :ms
 * @param {*} interval 默认：200 :ms
 */
export function debouncer(fn, param, time, interval = 200) {
